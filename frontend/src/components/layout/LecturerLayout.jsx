      ...
      <div className="flex h-screen overflow-hidden" style={{ backgroundColor: '#F0F0F2' }}>
        <aside className="flex flex-col h-full w-14 shrink-0 items-center py-4 gap-3"
          ...
          <div className="flex flex-col gap-2 mt-2">
            <button onClick={() => navigate('/lecturer/dashboard')} title="Home"
              className={`p-2 rounded-lg transition-colors ${isActive('/lecturer/dashboard') ? 'bg-white text-gray-900' : 'text-gray-500 hover:bg-white'}`}>
              <I.Home />
            </button>
            <button onClick={() => navigate('/lecturer/profile')} title="My Profile"
              className={`p-2 rounded-lg transition-colors ${isActive('/lecturer/profile') ? 'bg-white text-gray-900' : 'text-gray-500 hover:bg-white'}`}>
              <I.User />
            </button>
            <button onClick={() => navigate('/lecturer/courses')} title="My Courses"
              className={`p-2 rounded-lg transition-colors ${isActive('/lecturer/courses') ? 'bg-white text-gray-900' : 'text-gray-500 hover:bg-white'}`}>
              <I.Folder />
            </button>
          </div>
          ...
          <div className="flex flex-col gap-2 mt-1">
            {navItems.map(({ label, Icon, path }) => (
              <button key={path} onClick={() => navigate(path)} title={label}
                className={`p-2 rounded-lg transition-colors ${isActive(path) ? 'bg-white text-gray-900' : 'text-gray-500 hover:bg-white'}`}>
                <Icon />
              </button>
            ))}
          </div>
          ...
          <div className="flex flex-col items-center gap-2 mb-2">
            <button onClick={() => navigate('/lecturer/profile')} title={user?.full_name}
              className="h-7 w-7 rounded-full bg-gray-700 text-white flex items-center justify-center text-xs font-bold hover:opacity-80 transition-opacity">
              {user?.full_name?.[0] || 'L'}
            </button>
        ...
        <main className="flex-1 overflow-y-auto" style={{ backgroundColor: '#F0F0F2' }}>
